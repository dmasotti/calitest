#!/usr/bin/env bash
# End-to-end conflict scenario: server deletes, client pushes update (full sync)
set -euo pipefail
...
if [[ -z "$CALIMOB_SERVER_LIBRARY_ID" ]]; then
  SETUP_OUT=$(CALIBRE_LIBRARY_ID="$CALIMOB_LIBRARY_ID" \
    CALIMOB_LIBRARY_NAME="Headless Conflict Suite $(date -u +%Y%m%d%H%M%S)" \
...
TMP_CFG=$(mktemp -d /tmp/calimob_conflict_cfg_XXXXXX)
...
lm["$CALIMOB_LIBRARY_ID"] = {
    ...
    "calimobLibraryName": "Conflict Suite",
}
...
DELETE_RES=$(curl -s -X POST "$API_URL/sync" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "$DELETE_PAYLOAD")
if ! echo "$DELETE_RES" | jq -e '.results' >/dev/null 2>&1; then
  echo "FAIL: delete sync failed" >&2
  echo "$DELETE_RES" >&2
  exit 1
fi

# Local edit (ensure push sends update)
sqlite3 "$CALIMOB_LIBRARY_PATH/metadata.db" \
  "UPDATE books SET title='$TITLE_LOCAL', last_modified=datetime('now') WHERE id=$BOOK_ID;"

# Full sync: push first -> expect conflict reason 'deleted'
...
for i in range(len(out)-1, -1, -1):
    ...
    try:
        ...
        continue
if start is None:
    raise SystemExit("FAIL: no JSON summary in conflict run")
obj = json.loads(out[start:])
push = obj.get("push", {}) or {}
if push.get("errors"):
    raise SystemExit("FAIL: push errors in conflict run")
if int(push.get("conflicts", 0)) < 1:
    raise SystemExit("FAIL: expected conflict, got 0")
details = push.get("conflict_details", [])
if details and details[0].get("reason") != "deleted":
    raise SystemExit("FAIL: expected conflict reason 'deleted'")
print("PASS: conflict end-to-end (deleted) scenario")
PY
